...
ParticleRstOutput::ParticleRstOutput(ParameterInput *pin, Mesh *pm, OutputParameters op) :
  BaseTypeOutput(pin, pm, op) {
  // create new directory for this output. Comments in binary.cpp constructor explain why
  // mkdir("pvtk",0775);
}

void ParticleRstOutput::LoadOutputData(Mesh *pm) {
  particles::Particles *pp = pm->pmb_pack->ppart;
  npout_thisrank = pm->nprtcl_thisrank;
  npout_total = pm->nprtcl_total;
  Kokkos::realloc(outpart_rdata, pp->nrdata, npout_thisrank);
...
void ParticleRstOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  ...
  {
    std::stringstream msg;
    msg << "# rst DataFile Version 2.0" << std::endl
        << "# AthenaK particle data "
        << "  nranks= " << global_variable::nranks
        << "  nprtcls= " << pm->nprtcl_total << std::endl
        << "<head_end>" << std::endl;
