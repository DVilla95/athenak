...
void TorusFluxes(HistoryData *pdata, Mesh *pm) {
  ...
  for (int g=0; g<nradii; ++g) {
    ...
    for (int n=0; n<grids[g]->nangles; ++n) {
      ...
      Real theta = grids[g]->polar_pos.d_view(n,0);
      Real phi = grids[g]->polar_pos.d_view(n,1);
      Real x1 = grids[g]->interp_coord.d_view(n,0);
      Real x2 = grids[g]->interp_coord.d_view(n,1);
      Real x3 = grids[g]->interp_coord.d_view(n,2);
      Real glower[4][4], gupper[4][4];
      ComputeMetricAndInverse(x1,x2,x3,flat,spin,glower,gupper);

      // extract interpolated primitives
      Real &int_dn = grids[g]->interp_vals.d_view(n,IDN);
      Real &int_vx = grids[g]->interp_vals.d_view(n,IVX);
      Real &int_vy = grids[g]->interp_vals.d_view(n,IVY);
      Real &int_vz = grids[g]->interp_vals.d_view(n,IVZ);
      Real int_ie = grids[g]->interp_vals.d_view(n,IEN)*to_ien;

      // extract interpolated field components (iff is_mhd)
      Real int_bx = 0.0, int_by = 0.0, int_bz = 0.0;
      if (is_mhd) {
        int_bx = interpolated_bcc.d_view(n,IBX);
        int_by = interpolated_bcc.d_view(n,IBY);
        int_bz = interpolated_bcc.d_view(n,IBZ);
      }
      ...
      // integration params
      Real &domega = grids[g]->solid_angles.d_view(n);
